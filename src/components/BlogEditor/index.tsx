...
const BlogEditor: React.FC<BlogEditorProps> = ({ post, close, refresh }) => {
    ...
    const savePost = async () => {
        if (process.env.NODE_ENV === "development") {
            ...
            alert("This only works in development mode");
        }
    };
                ...
                {currentTabs === "BLOGDETAILS" && (
                    <div className="mt-10">
                        <div className="mt-5 flex flex-col items-center">
                            <label className="w-full text-sx opcaity-50">Date</label>
                            <DatePicker
                                selected={new Date(blogVariables.date)}
                                className="w-full mt-2 p-4 hover:border-blue-400 rounded-md shadow-lg boarder-2"
                                onChange={(date) => setBlogVariables({ ...blogVariables, date: date?.toISOString() ?? '' })}
                            />
                        </div>
                        <div className="mt-5 flex flex-col items-center">
                            <label className="w-full text-sx opcaity-50">Title</label>
                            <input
                        ...
                        <div className="mt-5 flex flex-col items-center">
                            <label className="w-full text-sx opcaity-50">Tagline</label>
                            <input
                        ...
                        <div className="mt-5 flex flex-col items-center">
                            <label className="w-full text-sx opcaity-50">Preview</label>
                            <input
                        ...
                        <div className="mt-5 flex flex-col items-center">
                            <label className="w-full text-sx opcaity-50">Image</label>
                            <input
                ...
                {currentTabs === "CONTENT" && (
                    <div className="mt-10">
                        <div className="flex flex-col items-center">
                            <label className="w-full text-sx opcaity-50">Content</label>
                            <TextareaAutoSize
                                value={blogContent}
                                onChange={(e) => setBlogContent(e.target.value)}
                                className="w-full mt-2 p-4 hover:border-blue-400 rounded-xl shadow-xl"
                            />
                        </div>
                    </div>
                )}
